...
import { useBatchSyncWalmartData, useWalmartConfigStatus, useWalmartSyncStats } from '@/hooks/useWalmartMarketData';
...
export function WalmartBatchSync() {
  ...
  const { data: configStatus, isLoading: isConfigStatusLoading } = useWalmartConfigStatus();
  const { data: syncStats, isLoading: isStatsLoading } = useWalmartSyncStats();
  const batchSyncMutation = useBatchSyncWalmartData();
  ...
  const handleSaveConfig = (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: 'Walmart API credentials are set as environment secrets. The integration is ready to use!',
    });
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center">
          ...
          Walmart Product Sync
        </CardTitle>
        <CardDescription>
          Sync product data with Walmart Selling Partner API
        </CardDescription>
      </CardHeader>
      <CardContent>
        {!isConfigStatusLoading && !isConfigValid ? (
          <Alert className="mb-4 border-yellow-500 bg-yellow-50">
            ...
            <AlertDescription className="text-yellow-700">
              Walmart API configuration is incomplete. Click the "Configure API" button to set up the required credentials.
            </AlertDescription>
          </Alert>
        ) : null}
      ...
      <CardFooter className="flex justify-between">
        <Dialog open={isConfigModalOpen} onOpenChange={setIsConfigModalOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Walmart API Configuration</DialogTitle>
              <DialogDescription>
                Enter your Walmart Seller API credentials. These will be saved as environment variables.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              <Alert>
                ...
                <AlertTitle>Walmart API Credentials</AlertTitle>
                <AlertDescription>
              ...
              {!(configStatus as { configValid?: boolean; missingEnvVars?: string[] } | undefined)?.configValid && (
                <Alert className="border-yellow-500 bg-yellow-50">
                  ...
                  <AlertDescription className="text-yellow-700">
                    Some credentials are not configured. Please add them in the Replit Secrets panel to enable Walmart integration.
                  </AlertDescription>
                </Alert>
              )}
                ...
                <ol className="list-decimal list-inside space-y-1">
                  ...
                  <li>Add or update these secrets:
                    <ul className="ml-6 mt-1 list-disc list-inside">
                      <li>WALMART_CLIENT_ID</li>
                      <li>WALMART_CLIENT_SECRET</li>
                    </ul>
                  </li>
