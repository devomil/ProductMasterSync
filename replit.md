...
- **Key Features**:
    ...
    - **Purchasing AI**: Multi-select fulfillment preferences, bulk analysis triggering with progress monitoring, and production-ready rate limiting with deduplication. Designed for 1M+ product catalogs.
    - **Research Opportunities**: Upload CSV/Excel product lists (UPCs, MPNs, keywords, ASINs) to discover marketplace selling opportunities. Parses diverse column formats (Ace overstock, Ingram Micro, generic). Runs each product against Amazon SP-API (UPC→ASIN lookup, competitive pricing, fees, restrictions) and calculates dropship/warehouse margins. Results table with opportunity badges, margin highlighting, restriction status, and Amazon links. Replaces old "Batch Processing" page. Backend: `POST /api/purchasing/upload-analyze`, `GET /api/purchasing/uploads`, `GET /api/purchasing/uploads/:id/results`.
    ...
    - **Ingram Micro Async Order Management**: When fulfilling via Ingram Micro, the modal offers a PO confirmation step showing items, ship-to address, and total vendor cost before submission. Creates purchase orders via `POST /resellers/v6/orders`. Tracks PO number (`purchaseOrderNumber`), vendor order status (`vendorOrderStatus`), and vendor order date on `marketplace_orders`. Tracks `ingramPartNumber`, `vendorTrackingNumber`, and `vendorCarrier` on `marketplace_order_items`. Includes live PO status checking via `GET /api/marketplace/orders/:id/vendor-order-status` and tracking refresh via `POST /api/marketplace/orders/:id/refresh-vendor-tracking`. Graceful degradation: if Ingram API call fails, fulfillment data is still saved locally. PO numbers and vendor order status badges displayed in orders table.
    ...
    - **Product Connection Visibility**: Product Details Supplier Info tab shows all data connections feeding each supplier. Displays connection type (SFTP, API, etc.), purpose badge (Catalog Import, Inventory & Pricing, Order Fulfillment, Catalog Search, Returns, General), automation status, and active/inactive state. Data fetched via enhanced `/api/products/:id/suppliers` endpoint that joins `data_sources` and `supplier_automation` tables per supplier.
...
- **Ingram Micro Resellers API v6**: OAuth2 via `/oauth/oauth30/token` endpoint. Product catalog search, price & availability, freight estimates, order creation (`POST /resellers/v6/orders`), order tracking, and invoice management. Used for order fulfillment vendor lookup, dropship PO submission, and as a data source type. Data sources support `purpose` field (catalog, inventory_pricing, order_fulfillment, catalog_search, returns, general).
