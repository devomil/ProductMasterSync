...
- **Core Features**:
    ...
    - **Field Mapping System**: Purpose-driven mapping templates with interactive walkthroughs and AI auto-mapping. Mapping workflow adapts based on data source purpose: Catalog Import shows all 60+ fields, Inventory & Pricing shows only ~13 fields (identifier + pricing + inventory), Order Fulfillment shows 5 fields, Catalog Search skips mapping entirely. Purpose stored on `mapping_templates` table and used by automation scheduler to determine update strategy. Mapping template lookup is supplier-aware: prefers templates matching both `sourceType` AND `supplierId`, falling back to type-only. Startup seeder ensures each supplier has its own SFTP mapping template. Source field dropdowns show sample data previews next to each field name. Supports **computed fields** (SUM, CONCAT, FIRST_NON_EMPTY) to combine multiple source columns into one target field — stored in mapping template as `{ computed: true, operation: "SUM", sourceFields: [...] }` and applied during import via `applyComputedField()`. Header detection heuristic recognizes both camelCase and UPPER_CASE_WITH_UNDERSCORES column names.
    ...
    - **Catalog Data Enrichment**: UI-driven enrichment system with four operations: (1) AI Auto-Categorize — uses Claude to batch-categorize products into IT/electronics hierarchy, (2) Enrich Descriptions — matches products to Walmart listings by UPC/SKU for better descriptions, (3) Discover Amazon ASINs — UPC lookup via Amazon SP-API to populate `product_asin_mapping`, (4) Link Walmart IDs — migrates `walmartItemId` from product attributes to `product_walmart_mapping` table + UPC/SKU matching. All accessible from "Data Enrichment" dropdown on Master Catalog page.
