...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Products API - Warning: This endpoint loads ALL products and impacts performance
  ...
  // Optimized Products Search API with pagination and filtering
    ...
    try {
      ...
      if (query && typeof query === 'string' && query.trim()) {
        ...
        if (searchType === 'sku') {
          ...
          conditions.push(`p.upc ILIKE $${paramIndex}`);
          params.push(`%${searchQuery}%`);
          paramIndex++;
        } else if (searchType === 'mfgPart') {
          conditions.push(`p.manufacturer_part_number ILIKE $${paramIndex}`);
          params.push(`%${searchQuery}%`);
          paramIndex++;
          ...
          // 'all' search type - search across multiple fields
          conditions.push(`(
            p.sku ILIKE $${paramIndex} OR 
            p.upc ILIKE $${paramIndex + 1} OR 
            p.name ILIKE $${paramIndex + 2} OR 
            p.manufacturer_part_number ILIKE $${paramIndex + 3} OR
      ...
      // Manufacturer filter (using manufacturer_part_number column that exists)
      ...
      // Boolean filters
      if (isRemanufactured === 'true') {
        conditions.push(`p.is_remanufactured = true`);
      }
      if (isCloseout === 'true') {
        conditions.push(`p.is_closeout = true`);
      }
      if (isOnSale === 'true') {
        conditions.push(`p.is_on_sale = true`);
      }
      if (hasRebate === 'true') {
        conditions.push(`p.has_rebate = true`);
      }
      ...
      if (inventoryStatus === 'inStock') {
        ...
        conditions.push(`p.inventory_quantity > 0 AND p.inventory_quantity <= 10`);
      } else if (inventoryStatus === 'outOfStock') {
        conditions.push(`p.inventory_quantity = 0 OR p.inventory_quantity IS NULL`);
      }

      const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';

      // Validate sort parameters
      const validSortFields = ['sku', 'name', 'price', 'cost', 'inventory_quantity', 'created_at'];
      const sortField = validSortFields.includes(sortBy as string) ? sortBy : 'sku';
      ...
      // Execute raw SQL queries using the pool
